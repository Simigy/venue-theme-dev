...
$(document).ready(function () {
    // Function to update the variant price when the select option changes
    $('.js-product-variant-select').on('change', function () {
      var selectedOption = $(this).find('option:selected');
      var currentVarPrice = selectedOption.data('variantprice');
      //  console.log(currentVarPrice);
      // Find the nearest span with the class .current__variant--price
      var priceElement = $(this).closest('.product__grid_select-box').find('.current__variant--price');

      if (priceElement.length > 0) {
        priceElement.text(currentVarPrice);
      }
    });

    // Trigger the change event on page load to set the initial price
    $('.js-product-variant-select').change();
  });